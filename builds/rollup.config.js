...
// 获取package.json,找到以 @hongye-ui 开头的
const inputs = getPackagesSync().map(pck => pck.name).filter(name => name.includes('@hongye-ui'));

export default inputs.map(name => {
    const pckName = name.split('@hongye-ui')[1] // button icon
    return {
